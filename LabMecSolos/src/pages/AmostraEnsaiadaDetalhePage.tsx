...
const AmostraEnsaiadaDetalhePage: React.FC = () => {
  const { id } = useParams<{ id: string }>();
  const history = useHistory();
  const { usuario } = useAuth();
  const [amostra, setAmostra] = useState<AmostraEnsaiada | null>(null);
  const [contexto, setContexto] = useState<{ pesquisaTitulo: string; pontoPlano: string; amostraCampo: string; preparadaNumero: string } | null>(null);
    ...
    try {
      ...
      if (!isNative) {
        ...
        input.onchange = async (e: Event) => {
          ...
          reader.onload = async () => {
            if (reader.result) {
              await ImagemService.upload('amostra_ensaiada', id, reader.result as string, null, usuario.userId);
              setGaleriaKey((k) => k + 1);
            }
          };
      ...
      if (photo.dataUrl) {
        await ImagemService.upload('amostra_ensaiada', id, photo.dataUrl, null, usuario.userId);
        setGaleriaKey((k) => k + 1);
      }
    } catch (e: unknown) {
  ...
  useEffect(() => {
    const load = async () => {
      if (!id) return;
      try {
        const ae = await AmostragemService.obterAmostraEnsaiada(id);
        setAmostra(ae);
        const ctx = await queryRows<{ pesquisa_titulo: string; ponto_plano: string; amostra_campo: string; preparada_numero: string }>(
          `SELECT pesq.titulo AS pesquisa_titulo, pc.identificacao_plano AS ponto_plano,
                  ab.numero_identificacao_campo AS amostra_campo, ap_ctx.numero_amostra AS preparada_numero
           FROM amostras_ensaiadas ae2
           INNER JOIN amostras_preparadas ap_ctx ON ae2.id_amostra_preparada = ap_ctx.id
           INNER JOIN amostras_brutas ab ON ap_ctx.id_amostra_bruta = ab.id
        ...
        if (ctx.length > 0) setContexto({ pesquisaTitulo: ctx[0].pesquisa_titulo, pontoPlano: ctx[0].ponto_plano, amostraCampo: ctx[0].amostra_campo, preparadaNumero: ctx[0].preparada_numero });
        const ens = await queryRows<{ id: string }>(
          'SELECT id FROM ensaios WHERE id_amostra_ensaiada = ?', [id]
        );
        if (ens.length > 0) setEnsaioExistente(ens[0]);
      } catch {
        setToastMsg('Erro ao carregar amostra fracionada.');
        setShowToast(true);
      }
      setLoading(false);
    };
  ...
  if (loading || !amostra) {
    return (
      <IonPage><AppBar title="Amostra Fracionada" /><IonContent><div style={{ display: 'flex', justifyContent: 'center', alignItems: 'center', minHeight: '60vh' }}><IonSpinner name="crescent" color="primary" /></div></IonContent></IonPage>
    );
  }
      ...
      <IonContent>
        <div style={{ padding: 16 }}>
            ...
            <IonCard style={{ borderRadius: 12, marginBottom: 12 }}>
              <IonCardContent>
                ...
                <div style={{ fontSize: 12, color: 'var(--ion-color-medium)' }}>
                  Ponto: {contexto.pontoPlano} &middot; Amostra: {contexto.amostraCampo} &middot; Preparada: {contexto.preparadaNumero}
                </div>
              </IonCardContent>
            </IonCard>
          )}
          ...
          <IonCard style={{ borderRadius: 12 }}>
            <IonCardContent>
              <IonItem lines="none"><IonLabel position="stacked"><small>Tipo de Ensaio Destino</small></IonLabel><div>{TIPO_ENSAIO_LABELS[amostra.tipo_ensaio_destino] || amostra.tipo_ensaio_destino}</div></IonItem>
              <IonItem lines="none"><IonLabel position="stacked"><small>Quantidade Inicial</small></IonLabel><div>{amostra.quantidade_inicial != null ? `${amostra.quantidade_inicial} g` : '—'}</div></IonItem>
              {amostra.quantidade_final != null && <IonItem lines="none"><IonLabel position="stacked"><small>Quantidade Final</small></IonLabel><div>{amostra.quantidade_final} g</div></IonItem>}
              {amostra.descricao && <IonItem lines="none"><IonLabel position="stacked"><small>Descrição</small></IonLabel><div>{amostra.descricao}</div></IonItem>}
          ...
          {amostra.finalizado === 0 && (
            <IonButton expand="block" fill="outline" style={{ marginTop: 8 }} onClick={() => history.push(`/app/ensaios/amostra-ensaiada/${id}/editar`)}>
              Editar
            </IonButton>
          )}
          {ensaioExistente ? (
...
export default AmostraEnsaiadaDetalhePage;
