...
const MinhasOcorrenciasPage: React.FC = () => {
  ...
  const carregar = useCallback(async (p: number, reset: boolean) => {
    setLoading(true);
    const result = await InventarioService.listarOcorrencias(filtros, p, usuario?.userId, true);
  ...
  return (
    <IonPage>
      <AppBar title="Minhas Ocorrencias" />
      <IonContent>
        ...
        {loading && ocorrencias.length === 0 ? (
          ...
          <EmptyState icon={flagOutline} title="Voce nao possui ocorrencias registradas." actionLabel="Nova Ocorrencia" onAction={() => history.push('/app/inventario/ocorrencias/nova')} />
        ) : (
          <>
            {ocorrencias.map((o) => (
...
export default MinhasOcorrenciasPage;
